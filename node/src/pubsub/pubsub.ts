...
export async function listenForMessages(subscriptionName: string, timeout = 60, cb: (m: string) => void) {
...
export async function CreateTopicSubscription() {
  ...
  const [sub] = await client.topic(topicName).createSubscription(subscriptionName);
